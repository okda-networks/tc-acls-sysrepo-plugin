...
onm_tc_ace_element_t* extract_ace_elements_with_change_ops(const onm_tc_ace_element_t* ace, const int change_op_set[], size_t set_size) {
	...
	if (is_updated){
		SRPLG_LOG_INF(PLUGIN_NAME, "[CHANGE EVENT] Change event extract change operation set success, ACE name %s, Priority %d",ace->ace.name,ace->ace.priority);
		...
		SRPLG_LOG_INF(PLUGIN_NAME, "[CHANGE EVENT] Change event extract change operation set, found no matching operation element, ACE Name %s, Priority %d",ace->ace.name,ace->ace.priority);
...
int apply_ace_created_operation(onm_tc_ctx_t * ctx, onm_tc_ace_element_t * ace, unsigned int acl_id){
	...
	if (created_ace == NULL){
		SRPLG_LOG_ERR(PLUGIN_NAME, "[CHANGE EVENT] Change operation 'ACE Created', ace element is NULL");
		return -1;
	}
	SRPLG_LOG_INF(PLUGIN_NAME, "[CHANGE EVENT] Apply change operation 'ACE Created' ACE Name %s, Priority %d",ace->ace.name,ace->ace.priority);
	ret = tcnl_filter_modify(ctx, created_ace,acl_id, RTM_NEWTFILTER,NLM_F_CREATE,true);
...
int apply_ace_deleted_operation(onm_tc_ctx_t * ctx, onm_tc_ace_element_t * ace, unsigned int acl_id){
	int ret = 0;
	if (ace == NULL) {
		SRPLG_LOG_ERR(PLUGIN_NAME, "[CHANGE EVENT] Change operation 'ACE Deleted', ace element is NULL");
		return -1;
	}

	SRPLG_LOG_INF(PLUGIN_NAME, "[CHANGE EVENT] Apply change operation 'ACE Deleted' ACE Name %s, Priority %d",ace->ace.name,ace->ace.priority);
	ret = tcnl_filter_modify(ctx, ace,acl_id,RTM_DELTFILTER,0,false);
	return ret;
}
...
int apply_ace_modified_operation(onm_tc_ctx_t * ctx, onm_tc_ace_element_t * ace, onm_tc_ace_element_t* running_ace, unsigned int acl_id){
	...
	if (modified_ace == NULL){
		SRPLG_LOG_ERR(PLUGIN_NAME, "[CHANGE EVENT] Change operation 'ACE Modified', ace element is NULL");
		return -1;
	}
	if (running_ace){
	...
	if (ret<0){
		SRPLG_LOG_ERR(PLUGIN_NAME, "[CHANGE EVENT] Change operation 'ACE Deleted' failed, ACE Name %s, Priority %d",ace->ace.name,ace->ace.priority);
	}
	SRPLG_LOG_INF(PLUGIN_NAME, "[CHANGE EVENT] Apply change operation 'ACE Modifed' ACE Name %s, Priority %d",ace->ace.name,ace->ace.priority);
	ret = tcnl_filter_modify(ctx, modified_ace,acl_id,RTM_NEWTFILTER,NLM_F_CREATE,true);
...
int apply_events_ace_changes(onm_tc_ctx_t * ctx, const char * acl_name, unsigned int acl_id, onm_tc_ace_element_t* ace){
	...
	switch (ace->ace.name_change_op) {
		case SR_OP_CREATED:
			...
			SRPLG_LOG_INF(PLUGIN_NAME, "[CHANGE EVENT] Apply 'ACE Created' operation, ACE Name %s, Priority %d",ace->ace.name,ace->ace.priority);
		...
		case SR_OP_DELETED: {
				...
				if(!running_ace){
					SRPLG_LOG_ERR(PLUGIN_NAME, "[CHANGE EVENT] Apply 'ACE Delete' operation, Failed to get running ace %s",ace->ace.name);
					return -1;
				}
				ret = apply_ace_deleted_operation(ctx,running_ace,acl_id);
				if (ret < 0){
