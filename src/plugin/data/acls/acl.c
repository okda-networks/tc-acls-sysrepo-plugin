...
void onm_tc_acl_hash_element_free(onm_tc_acl_hash_element_t** el)
{
    if (*el) {
        ...
        // TODO fix data type
        //if ((*el)->acl.ace.actions.forwarding) {
            //ONM_TC_ACL_LIST_FREE((*el)->acl.ace.actions.forwarding);
        //}
        // element data
        free(*el);
        *el = NULL;
...
int onm_tc_acl_hash_from_ly(onm_tc_acl_hash_element_t** acl_hash, const struct lyd_node* acl_list_node)
{
    ...
    while (acl_iter) {
        ...
        //set data
        //TODO netlink set TC configuration if needed
        //First we need to evaluate if this needs to be done here or at attachment points config.
        if (acl_name_node){
