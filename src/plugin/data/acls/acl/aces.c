...
int ensure_ace_exists_in_acls_list(onm_tc_acl_hash_element_t** acl_hash, const char * acl_name, const char * ace_name){
    ...
    if (!updated_acl){
        // add new acl data
        SRPLG_LOG_DBG(PLUGIN_NAME, "Change event ACL name %s is not present in events acl hash, creating new ACL element.",acl_name);
    ...
    if (!updated_ace){
        SRPLG_LOG_DBG(PLUGIN_NAME, "[%s] Change event ACE name %s is not present in events acls hash, creating new ACE element.",acl_name, ace_name);
        updated_ace = onm_tc_ace_hash_element_new();
        SRPLG_LOG_DBG(PLUGIN_NAME, "[%s] Change event didn't happen on ACE name %s, setting ACE name change operation to default.",acl_name, ace_name);
...
int reorder_events_acls_aces_from_change_ctx(void *priv, sr_session_ctx_t *session, const srpc_change_ctx_t *change_ctx){
    ...
    if (strcmp(node_name,"ace")==0){
        ...
        SRPLG_LOG_INF(PLUGIN_NAME, "[%s][%s] Validate ACEs priority", acl_name_buffer, ace_name_buffer);
        if (change_ctx->operation == SR_OP_CREATED || change_ctx->operation == SR_OP_MOVED || change_ctx->operation == SR_OP_DELETED){
            ...
            SRPLG_LOG_DBG(PLUGIN_NAME, "[PRIO VALIDATION] Copying running acls list to events acls list");
            ...
            SRPLG_LOG_DBG(PLUGIN_NAME, "[PRIO VALIDATION] Reording aces in events list");
            ...
            SRPLG_LOG_DBG(PLUGIN_NAME, "[PRIO VALIDATION] Setting events list aces priority values");
...
int events_acls_hash_update_ace_element_from_change_ctx(void *priv, sr_session_ctx_t *session, const srpc_change_ctx_t *change_ctx)
{
    ...
    if (node_value){
        ...
        {
            // add new acl data
            SRPLG_LOG_DBG(PLUGIN_NAME, "Change event ACL name %s is not present in events acl hash, creating new ACL element.",acl_name_buffer);
            updated_acl = onm_tc_acl_hash_element_new();
        ...
        if (!updated_ace)
        {
            SRPLG_LOG_DBG(PLUGIN_NAME, "[%s] Change event ACE name %s is not present in events acls hash, creating new ACE element.",acl_name_buffer,ace_name_buffer);
            updated_ace = onm_tc_ace_hash_element_new();
            // if the change event happened on the ace name.
            if (strcmp(node_name,"name")==0){
                SRPLG_LOG_DBG(PLUGIN_NAME, "[%s] Change event happned on ACE name %s, setting ACE name operation to %d.",acl_name_buffer,ace_name_buffer,change_ctx->operation);
                error = onm_tc_ace_hash_element_set_ace_name(&updated_ace,node_value,change_ctx->operation);
                if (error) return error;
            }
            else {
                SRPLG_LOG_DBG(PLUGIN_NAME, "[%s] Change event didn't happen on ACE name %s, setting ACE name change operation to default.",acl_name_buffer,ace_name_buffer);
            ...
            if (running_ace){
                ...
                SRPLG_LOG_DBG(PLUGIN_NAME, "[%s] Change event ACE name %s has no corresponding ACE in running acls list, setting ACE priority to 0.",acl_name_buffer,ace_name_buffer);
        ...
        error = ace_element_update_from_lyd_node(updated_ace,node,change_ctx->operation,acl_name_buffer);
...
int ace_element_update_from_lyd_node(onm_tc_ace_element_t* updated_ace,const struct lyd_node * node, sr_change_oper_t change_operation, char * acl_name) {
    ...
    if (strcmp(node_name,"name") != 0){
     SRPLG_LOG_DBG(PLUGIN_NAME, "[%s] Update change ACE %s element '%s' value %s, change operation %d.",acl_name,updated_ace->ace.name,node_name,node_value,change_operation);
    }
    ...
    //L4
    {
        if (strcmp(parent_node_name,"source-port")==0){
            if (strcmp(grand_parent_node_name,"tcp")==0){
                if (strcmp(node_name,"operator")==0){ // tcp source port, single Port Operator
                    ...
                    error = port_str_to_port_attr(port_attr,NULL,NULL,NULL,port_opr,PORT_ATTR_SRC,PORT_ATTR_PROTO_TCP);
                    if (error) {
                        free(port_attr);
                        goto port_attr_error_out;
                    }
                ...
                if (strcmp(node_name,"port")==0){ // tcp source port, single port value
                    onm_tc_port_attributes_t *port_attr = malloc(sizeof(onm_tc_port_attributes_t));
                    error = port_str_to_port_attr(port_attr,NULL,NULL,node_value,PORT_NOOP,PORT_ATTR_SRC,PORT_ATTR_PROTO_TCP);
                    if (error) {
                        free(port_attr);
                        goto port_attr_error_out;
                    }
                ...
                if (strcmp(node_name,"lower-port")==0 ){ // tcp source port, port range
                    onm_tc_port_attributes_t *port_attr = malloc(sizeof(onm_tc_port_attributes_t));
                    error = port_str_to_port_attr(port_attr,node_value,NULL,NULL,PORT_NOOP,PORT_ATTR_SRC,PORT_ATTR_PROTO_TCP);
                    if (error) {
                        free(port_attr);
                        goto port_attr_error_out;
                    }
                ...
                if (strcmp(node_name,"upper-port")==0){ // tcp source port, port range
                    onm_tc_port_attributes_t *port_attr = malloc(sizeof(onm_tc_port_attributes_t));
                    error = port_str_to_port_attr(port_attr,NULL,node_value,NULL,PORT_NOOP,PORT_ATTR_SRC,PORT_ATTR_PROTO_TCP);
                    if (error) {
                        free(port_attr);
                        goto port_attr_error_out;
                    }
                ...
                if (strcmp(node_name, "port") == 0) {
                    onm_tc_port_attributes_t *port_attr = malloc(sizeof(onm_tc_port_attributes_t));
                    error = port_str_to_port_attr(port_attr,NULL,NULL,node_value,PORT_NOOP,PORT_ATTR_SRC, PORT_ATTR_PROTO_UDP);
                    if (error) {
                        free(port_attr);
                        goto port_attr_error_out;
                    }
                ...
                if (strcmp(node_name, "operator") == 0) {
                    onm_tc_port_attributes_t *port_attr = malloc(sizeof(onm_tc_port_attributes_t));
                    port_operator_t port_opr = onm_tc_ace_port_oper_a2i(node_value);
                    error = port_str_to_port_attr(port_attr,NULL,NULL,NULL,port_opr,PORT_ATTR_SRC, PORT_ATTR_PROTO_UDP);
                    if (error) {
                        free(port_attr);
                        goto port_attr_error_out;
                    }
                ...
                if (strcmp(node_name, "lower-port") == 0) {
                    onm_tc_port_attributes_t *port_attr = malloc(sizeof(onm_tc_port_attributes_t));
                    error = port_str_to_port_attr(port_attr,node_value,NULL,NULL,PORT_NOOP,PORT_ATTR_SRC, PORT_ATTR_PROTO_UDP);
                    if (error) {
                        free(port_attr);
                        goto port_attr_error_out;
                    }
                ...
                if (strcmp(node_name, "upper-port") == 0) {
                    onm_tc_port_attributes_t *port_attr = malloc(sizeof(onm_tc_port_attributes_t));
                    error = port_str_to_port_attr(port_attr,NULL,node_value,NULL,PORT_NOOP, PORT_ATTR_SRC, PORT_ATTR_PROTO_UDP);
                    if (error) {
                        free(port_attr);
                        goto port_attr_error_out;
                    }
        ...
        if (strcmp(grand_parent_node_name, "tcp") == 0) {
            if (strcmp(node_name, "operator") == 0) {
                onm_tc_port_attributes_t *port_attr = malloc(sizeof(onm_tc_port_attributes_t));
                port_operator_t port_opr = onm_tc_ace_port_oper_a2i(node_value);
                error = port_str_to_port_attr(port_attr,NULL,NULL,NULL,port_opr,  PORT_ATTR_DST, PORT_ATTR_PROTO_TCP);
                if (error) {
                    free(port_attr);
                    goto port_attr_error_out;
                }
            ...
            if (strcmp(node_name, "port") == 0) {
                onm_tc_port_attributes_t *port_attr = malloc(sizeof(onm_tc_port_attributes_t));
                error = port_str_to_port_attr(port_attr,NULL,NULL,node_value,PORT_NOOP, PORT_ATTR_DST, PORT_ATTR_PROTO_TCP);
                if (error) {
                    free(port_attr);
                    goto port_attr_error_out;
                }
            ...
            if (strcmp(node_name, "lower-port") == 0) {
                onm_tc_port_attributes_t *port_attr = malloc(sizeof(onm_tc_port_attributes_t));
                error = port_str_to_port_attr(port_attr,node_value,NULL,NULL,PORT_NOOP,  PORT_ATTR_DST, PORT_ATTR_PROTO_TCP);
                if (error) {
                    free(port_attr);
                    goto port_attr_error_out;
                }
            ...
            if (strcmp(node_name, "upper-port") == 0) {
                onm_tc_port_attributes_t *port_attr = malloc(sizeof(onm_tc_port_attributes_t));
                error = port_str_to_port_attr(port_attr,NULL,node_value,NULL,PORT_NOOP, PORT_ATTR_DST, PORT_ATTR_PROTO_TCP);
                if (error) {
                    free(port_attr);
                    goto port_attr_error_out;
                }
            ...
            if (strcmp(node_name, "port") == 0) {
                onm_tc_port_attributes_t *port_attr = malloc(sizeof(onm_tc_port_attributes_t));
                error = port_str_to_port_attr(port_attr,NULL,NULL,node_value,PORT_NOOP, PORT_ATTR_DST, PORT_ATTR_PROTO_UDP);
                if (error) {
                    free(port_attr);
                    goto port_attr_error_out;
                }
            ...
            if (strcmp(node_name, "lower-port") == 0) {
                onm_tc_port_attributes_t *port_attr = malloc(sizeof(onm_tc_port_attributes_t));
                error = port_str_to_port_attr(port_attr,node_value,NULL,NULL,PORT_NOOP, PORT_ATTR_DST, PORT_ATTR_PROTO_UDP);
                if (error) {
                    free(port_attr);
                    goto port_attr_error_out;
                }
            ...
            if (strcmp(node_name, "upper-port") == 0) {
                onm_tc_port_attributes_t *port_attr = malloc(sizeof(onm_tc_port_attributes_t));
                error = port_str_to_port_attr(port_attr,NULL,node_value,NULL,PORT_NOOP, PORT_ATTR_DST, PORT_ATTR_PROTO_UDP);
                if (error) {
                    free(port_attr);
                    goto port_attr_error_out;
                }
...
void onm_tc_ace_hash_print_debug(const onm_tc_ace_element_t* ace_iter)
{
        ...
        if(ace_iter->ace.matches.eth.source_address_mask){
            SRPLG_LOG_DBG(PLUGIN_NAME, "| \t|\t|     |---- Source mac address mask = %s (change operation %d, set flag %d)",
        ...
        if(ace_iter->ace.matches.eth.destination_address){
            SRPLG_LOG_DBG(PLUGIN_NAME, "| \t|\t|     |---- Destination mac address = %s (change operation %d, set flag %d)",
        ...
        if(ace_iter->ace.matches.eth.destination_address_mask){
            SRPLG_LOG_DBG(PLUGIN_NAME, "| \t|\t|     |---- Destination mac address mask = %s (change operation %d, set flag %d)",
        ...
        if(ace_iter->ace.matches.ipv4.source_network){
            SRPLG_LOG_DBG(PLUGIN_NAME, "| \t|\t|     |---- Source IPv4 Network = %s (change operation %d, set flag %d)",
        ...
        if(ace_iter->ace.matches.ipv4.destination_network){
            SRPLG_LOG_DBG(PLUGIN_NAME, "| \t|\t|     |---- Destination IPv4 Network = %s (change operation %d, set flag %d)",
        ...
        if(ace_iter->ace.matches.ipv6.source_network){
            SRPLG_LOG_DBG(PLUGIN_NAME, "| \t|\t|     |---- Source IPv6 Network = %s (change operation %d, set flag %d)",
        ...
        if(ace_iter->ace.matches.ipv6.destination_network){
            SRPLG_LOG_DBG(PLUGIN_NAME, "| \t|\t|     |---- Destination IPv6 Network = %s (change operation %d, set flag %d)",
        ...
        SRPLG_LOG_DBG(PLUGIN_NAME, "| \t|\t|     + Actions:");{
            SRPLG_LOG_DBG(PLUGIN_NAME, "| \t|\t|     |---- Action-Forwarding = %d (change operation %d)",
            ace_iter->ace.actions.forwarding,
            ace_iter->ace.actions.forwarding_change_op);
        }
        if(ace_iter->ace.actions.logging != 0){
            SRPLG_LOG_DBG(PLUGIN_NAME, "| \t|\t|     |---- Action-Logging = %d (change operation %d)", 
